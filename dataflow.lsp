...
(defun c:modifyEquipmentProperty (/ equipmentPropertyNameListList dataTypeList)
  (setq equipmentPropertyNameListList (GetReactorPropertyNameList))
  (filterAndModifyEquipmentPropertyByBox equipmentPropertyNameListList "filterAndModifyEquipmentPropertyBox" "Reactor")
)
...
(defun filterAndModifyEquipmentPropertyByBox (propertyNameList tileName dataType / dcl_id propertyName propertyValue filterPropertyName patternValue replacedSubstring status selectedName selectedFilterName ss sslen matchedList importedList confirmList blockDataList entityNameList viewPropertyName previewDataList importedDataList exportMsgBtnStatus importMsgBtnStatus modifyMsgBtnStatus)
  (setq dcl_id (load_dialog (strcat "D:\\dataflowcad\\" "dataflow.dcl")))
  (setq status 2)
  (while (>= status 2)
    ; Create the dialog box
