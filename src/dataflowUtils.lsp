...
(defun GetBlockSSBySelectByDataTypeUtils (dataType / ss) 
  ...
  (if (= dataType "InstrumentAndEquipmentAndPipe")
    (setq ss (GetInstrumentPipeEquipSSBySelectUtils))
  ) 
  (if (= dataType "InstrumentAndPipe")
    (setq ss (GetInstrumentAndPipeSSBySelectUtils))
  )
  (if (= dataType "InstrumentAndEquipment")
    (setq ss (GetInstrumentAndEquipSSBySelectUtils))
  ) 
...
(defun GetAllBlockSSByDataTypeUtils (dataType / ss) 
  ...
  (if (= dataType "InstrumentAndEquipmentAndPipe")
    (setq ss (GetAllInstrumentPipeEquipSSUtils))
  )
  (if (= dataType "InstrumentAndPipe")
    (setq ss (GetAllInstrumentAndPipeSSUtils))
  )
  (if (= dataType "InstrumentAndEquipment")
    (setq ss (GetAllInstrumentAndEquipSSUtils))
  )
  (if (= dataType "EquipmentAndPipe")
...
(defun GenerateEquipTagText (insPt textContent /)
  (entmake (list (cons 0 "TEXT") (cons 100 "AcDbEntity") (cons 67 0) (cons 410 "Model") (cons 8 "DataFlow-GsLcEquipTag") (cons 100 "AcDbText") 
